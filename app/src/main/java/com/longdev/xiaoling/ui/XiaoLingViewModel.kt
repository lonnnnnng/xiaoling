...
import com.longdev.xiaoling.agent.NotificationPersonalNotePolicy
import com.longdev.xiaoling.agent.NotificationPersonalNoteSource
import com.longdev.xiaoling.agent.NotificationPersonalNoteSourceStatus
...
class XiaoLingViewModel(application: Application) : AndroidViewModel(application),
    ...
    private var pendingNotificationNoteSource: NotificationPersonalNoteSource? = null
    ...
    var uiState by mutableStateOf(
        initialUiState(
            ...
            answerabilityShadowSampleSummary = answerabilityShadowSampleTracker.snapshot(),
        ),
    )
        private set
    ...
    fun updatePrompt(value: String) {
        // long: 用户编辑通知生成的笔记草稿后，原通知只保留为来源提示，不再把编辑后的内容误绑定为通知原文。
        pendingNotificationNoteSource = null
    ...
    fun createAgentNoteDraftFromNotification(
        notificationId: String,
        onPrepared: () -> Unit,
    ) {
        if (
            ...
            showValidation("当前操作结束后再保存通知")
            return
        }
        val requestId = "notification-note-draft-${UUID.randomUUID()}"
        val conversationId = uiState.selectedConversationId
        notificationTaskDraftRequestId = requestId
        pendingNotificationTaskSource = null
        pendingNotificationNoteSource = null
        ...
        sendMessageJob = viewModelScope.launch {
            try {
                ...
                val notification = (readResult as? NotificationReadResult.Success)?.notification
                val draft = notification?.let(NotificationPersonalNotePolicy::createDraft)
                if (draft == null) {
                    showValidation(notificationNoteDraftFailureMessage(readResult))
                    return@launch
                }
                // long: 只把当前通知投影为可编辑 notes.create 草稿；用户仍需主动发送、审批和从当前笔记回读。
                pendingNotificationNoteSource = draft.source
                uiState = uiState.copy(
                    prompt = draft.prompt,
                    sharedDraftImported = false,
                    personalTaskMode = false,
                ...
                // long: 离开会话时撤销瞬时通知读取，不把未发送的来源投影到下一次编辑。
            } catch (error: Throwable) {
                if (notificationTaskDraftRequestId == requestId) {
                    showValidation(error.message ?: "无法重新读取当前通知")
                }
    ...
    private fun notificationNoteDraftFailureMessage(result: NotificationReadResult): String = when (result) {
        NotificationReadResult.AccessNotGranted -> "通知访问权限已撤销，无法保存为笔记"
        NotificationReadResult.ListenerDisconnected -> "通知监听服务未连接，无法保存为笔记"
        NotificationReadResult.NotFound -> "当前通知已消失或不可读取"
        is NotificationReadResult.Success -> if (result.notification.contentHidden) {
            "当前通知包含敏感或私密内容，不能保存为笔记"
        } else {
            "当前通知没有可保存的标题或正文"
        }
    }
    ...
    private suspend fun executeForegroundWorkflow(
        ...
        while (true) {
            ...
            val summary = agentRunUseCase.run(
                ...
                requiredSkillIds = if (input.targetAppPackage != null) {
                    setOf("device-control")
                } else {
                    emptySet()
                },
        ...
        when (event) {
            is ConversationSelectionEvent.DeletionStarted -> {
                ...
                recordAnswerabilityShadowNoticePruned(
                    ...
                    activeNoticeCount = nextState.answerabilityNotices.size,
                )
            }
    ...
    private fun sendAgentRun(
        ...
        val notificationNoteSource = pendingNotificationNoteSource
        pendingNotificationNoteSource = null
        sendMessageJob = viewModelScope.launch {
            try {
                conversationPersistenceCoordinator.cancelPendingSaveAndJoin()
                ...
                val approvalGate = interactiveAgentApprovalGate(
                    conversationId = conversationId,
                    notificationNoteSource = notificationNoteSource,
                )
    ...
    private suspend fun startCommittedTaskRetryIfPresent(
        ...
        val reusedStepCount = retryDetail.steps.count { step ->
        ...
        try {
            ...
            val completedRun = executeForegroundWorkflow(retryDetail, runtimeSelection, conversationId)
            appendTaskRetryCompletionMessage(
                conversationId = conversationId,
            ...
            val cancelledRun = withContext(NonCancellable + Dispatchers.IO) {
                workflowRepository.completeRun(
                    retryDetail.run.id,
                    WorkflowRunStatus.CANCELLED,
            ...
            val failedRun = withContext(NonCancellable + Dispatchers.IO) {
                workflowRepository.completeRun(
                    retryDetail.run.id,
                    WorkflowRunStatus.FAILED,
    ...
    private fun appendTaskRetryCompletionMessage(
        conversationId: String,
        taskName: String,
        status: WorkflowRunStatus,
        reusedStepCount: Int,
    ) {
        val presentation = presentTaskRetryCompletion(taskName, status, reusedStepCount) ?: return
        // long: 终态结果只来自 Repository 已收敛的关联 Run；工具的“已排队”回执不能被误报为任务已完成。
        appendWorkflowMessage(
            conversationId,
            ChatMessage(
    ...
    private fun interactiveAgentApprovalGate(
        conversationId: String,
        ...
        return object : ApprovalGate {
            override suspend fun requestApproval(
                ...
                if (toolCall.name == "notes.create" && notificationNoteSource != null) {
                    val current = (AndroidNotificationReader(getApplication()).get(notificationNoteSource.notificationId)
                        as? NotificationReadResult.Success)?.notification
                    val sourceStatus = current?.let {
                        NotificationPersonalNotePolicy.validateSource(notificationNoteSource, it)
                    }
                    if (sourceStatus != NotificationPersonalNoteSourceStatus.MATCHES) {
                        // long: 笔记写入审批前再次核对当前通知；来源消失或漂移时不创建审批卡，也不允许 notes.create 产生副作用。
                        return ApprovalDecision(
                            approved = false,
                            reason = "当前通知已变化或不可读取，未保存为笔记",
                        )
                    }
                }
    ...
    private fun publishAgentAnswerabilityShadow(
        ...
        // long: 只有用户显式开启的前台 shadow 才进入样本分母，关闭状态不计数以免稀释真实成本与失败率。
        if (!uiState.answerabilityShadowEnabled) return
        ...
        if (mode == KnowledgeAnswerabilityShadowObservationMode.DISABLED) {
            recordAnswerabilityShadowSample(
                KnowledgeAnswerabilityShadowSampleEvent(
                    kind = KnowledgeAnswerabilityShadowSampleKind.IDENTITY_MISMATCH,
                ),
            )
            return
        }
        val candidate = verifiedContext.latestKnowledgeAnswerabilityCandidate(question) ?: run {
            recordAnswerabilityShadowSample(
                KnowledgeAnswerabilityShadowSampleEvent(KnowledgeAnswerabilityShadowSampleKind.CANDIDATE_MISSING),
            )
            return
        }
        ...
        val publisher = AgentAnswerabilityShadowPublisher(
            ...
            publishSample = ::recordAnswerabilityShadowSample,
        )
            ...
            publisher.publish(
                ...
                awaitAnswerPersistence = {
                    ...
                    if (persistenceJob.isCancelled) {
                        throw CancellationException("答案保存已被更新快照取消")
                    }
    ...
    private fun publishAnswerabilityNotice(
        ...
        val previousNoticeIds = uiState.answerabilityNotices.keys
        val previousNotice = uiState.answerabilityNotices[persistedMessageId]
        val nextState = uiState.withAnswerabilityNotice(persistedMessageId, notice)
        val accepted = nextState.answerabilityNotices[persistedMessageId] == notice
        val prunedCount = (previousNoticeIds - nextState.answerabilityNotices.keys).size
        uiState = nextState
        if (accepted && previousNotice != notice) {
            recordAnswerabilityShadowNoticePublished(nextState.answerabilityNotices.size)
        }
        recordAnswerabilityShadowNoticePruned(
    ...
    private fun recordAnswerabilityShadowSample(event: KnowledgeAnswerabilityShadowSampleEvent) {
        // long: 旁路统计仅更新进程内 UI 摘要；任何聚合器异常都不能把已完成的 Agent 答案变成失败。
        runCatching {
            uiState = uiState.copy(
    ...
    private fun recordAnswerabilityShadowNoticePublished(activeNoticeCount: Int) {
        runCatching {
            uiState = uiState.copy(
                answerabilityShadowSampleSummary = answerabilityShadowSampleTracker
                    .recordNoticePublished(activeNoticeCount),
            )
        }
    }

    private fun recordAnswerabilityShadowNoticePruned(
        prunedCount: Int,
        ...
        runCatching {
            uiState = uiState.copy(
                answerabilityShadowSampleSummary = answerabilityShadowSampleTracker
                    .recordNoticePruned(prunedCount, activeNoticeCount),
            )
        }
    }
    ...
    private fun initialUiState(
        ...
        answerabilityShadowSampleSummary: KnowledgeAnswerabilityShadowSampleSummary,
    ): XiaoLingUiState {
        ...
        return StoredProfiles(
            ...
            .copy(
                ...
                answerabilityShadowSampleSummary = answerabilityShadowSampleSummary,
            )
    }
