...
object AgentTaskRetryPolicy {
    fun evaluate(detail: AgentRunDetailRecord): AgentTaskRetryEligibility {
        ...
        return if (detail.snapshot.run.status in retryableStatuses) {
            val successfulOrUncertainSideEffect = AgentTaskRetryEvidencePolicy
                .hasSuccessfulOrUncertainSideEffect(detail)
            ...
            AgentTaskRetryEligibility.Retryable(
                requiresConfirmation =
                    successfulOrUncertainSideEffect || interruptedDuringSideEffect || interruptedToolStep,
            )
        } else {
