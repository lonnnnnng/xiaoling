...
- Workflow 待完成生产接线：`device.swipe`。第 122/123/124/125 阶段已依次完成纯策略契约、Controller/Registry 执行期 HMAC evidence、完成态纯内存交接、Redmi 限定滚动，以及答案级脱敏 Decision、Room/Workflow output 与 UI 投影；同窗内容变化与共同匿名锚点方向主位移已经替代 generation-only 验证。当前只剩把该工具加入生产默认集合，并仅用 Redmi 完成真实生产 Workflow `snapshot -> swipe` 验收；完成前生产工具面继续保持六项。第 121 阶段 `open_app` 以聚焦 JVM `95/95`、Debug/AndroidTest APK、Redmi Compose/Room 单项、真实逐包审批 tracer 和文档 corpus 首轮 `OK (1 test)` 完成生产闭环；冻结 corpus 复验同样通过。
