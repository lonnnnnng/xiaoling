...
小灵在 `v0.1.9` 之后的当前 `main` 已具备可执行应用内任务的最小个人 Agent：普通聊天与 `/agent` 分流，Runtime 可取消、可限步、可确认、可验证并记录 Run、Step、Approval、Event 和 Memory；Agent Profile v1 已分离身份与能力，Room v24 Text/Reasoning/Image/Tool parts 已让自然语言、用户图片、供应商推理摘要与可信工具结果在同一消息内持久化和恢复。长期记忆、声明式 Skill、1 至 8 步 Workflow 与 WorkManager 非精确定时均已交付。用户 Image part 已完成真实 `gpt-5.5 + Responses` Redmi 验收；Document part 与通用旧执行栈续跑仍未交付。
...
- Text/Reasoning/Image/Tool 消息 parts：历史/流式文本兼容、单张用户图片、供应商 summary 折叠展示、Tool 可信投影、前后台统一 Room 写入和同气泡证据展示。
...
当前状态：请求取消、停止生成、Room 迁移、Schema 导出、v4→v24 迁移测试源码、Text/Reasoning/Image/Tool 消息 parts、Repository、Responses API 结构化文本/图片历史、函数 typed Items、可选 Reasoning summary、`LlmProviderAdapter` 和面向用户的 Room ZIP 备份/恢复已完成；ViewModel 继续瘦身仍待完成。
...
- 已完成：启用 Room Schema 导出，并为带旧数据的 v4→v24 migration 链、event metadata、Run 重试、Memory FTS、候选表、生命周期、Skill、Workflow、调度、多步骤快照、笔记幂等键、记忆 operation ledger/结果快照、独立工具账本、Agent Profile 和 MessagePart 提供自动化测试。
...
当前状态：`/agent` 最多 4 步的顺序工具闭环、运行预算、超时、取消、逐步审批、后置验证、多工具可信上下文、Run 时间线、RunEvent typed metadata、独立 ToolCall/ToolResult Room Ledger、可操作任务中心、安全重新运行和第一批应用内工具已完成；执行/验证中断默认采用 Run/活动 Step 一致取消和关联新 Run 重试。独立账本承接 v20 新事件的原子双写，任务中心、`notes.create / memory.remember` 受限验证恢复和失败 Run 重试副作用判断均已切换为 Ledger-first；账本异常时重试 fail-safe 要求确认，账本完全为空的旧 Run 保守回退 typed RunEvent。并行调用与通用原地断点恢复继续关闭。
...
| 优先级 | 工作项 | 当前状态 | 原因 |
|---|---|---|---|
| P0 | 请求取消、结构化 Responses 输入、Provider Adapter | 已完成，包括用户 Image、函数调用与结果 typed Items、可选 Reasoning summary；File Item 待后续扩展 | 后续 Agent 循环的基础协议 |
| P0 | Room、Repository、迁移测试和导出 | Room/Repository、Schema 导出、v4→v24、event metadata、重试关联、Memory FTS、候选表、生命周期、Skill/Workflow/调度/步骤定义表迁移、笔记幂等索引、记忆 operation ledger/结果快照、独立工具账本、Agent Profile、MessagePart 和用户 ZIP 备份/恢复已完成 | 保证升级和本地数据可恢复 |
...
| P1 | 结构化消息 parts | Text/Reasoning/Image/Tool 持久化、旧 text 回填、供应商摘要折叠展示、可信 Tool 投影、用户图片选择/预览/请求/备份和 Compose 展示已完成；Document 待后续 | 让聊天内容、用户附件、供应商摘要与工具执行事实进入同一可恢复消息模型 |
...
| P2 | 附件、视觉、语音和 RAG | 单张用户 Image 已完成；Document、语音和 RAG 未开始 | 提升输入输出能力 |
...
下一阶段建议沿用 Image 已验证的生命周期和信任边界，评估 Document part 的 MIME 白名单、页数/字符预算、原始文件与提取文本的双轨存储、Responses `input_file` 协议和 Compose 展示。Document 同样不能直接扩大 Agent 工具权限，也不能把模型描述提升为工具事实。
