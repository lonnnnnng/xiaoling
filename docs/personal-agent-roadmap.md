...
27. 下一步继续按真实使用证据处理计划等待/失败、常用任务闭环或单个限定 App 兼容；没有明确场景时不连续堆模板/App，不回到持续按行数拆分或单纯 Shadow 扩样。
28. 截图/视觉、后台设备控制、任意 App、精确定时、Foreground Service、MCP、系统日历、远程 Channel、多 Agent、跨设备同步和本地模型保持后置，并分别依据真实需求与证据立项。
