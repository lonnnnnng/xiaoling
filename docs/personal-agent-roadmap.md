...
- ViewModel 仍然过重；第 66 阶段已迁出普通聊天上下文准备，后续继续迁出网络发送状态和其他运行编排逻辑。
...
当前状态：请求取消、停止生成、Room 迁移、Schema 导出、v4→v29 迁移测试、Text/Reasoning/Image/Document/Tool 消息 parts、KnowledgeReference、独立进程退出观察、Repository、Responses API 结构化文本/附件历史、函数 typed Items、可选 Reasoning summary、`LlmProviderAdapter` 和面向用户的 Room ZIP 备份/恢复已完成；ViewModel 继续瘦身仍待完成。
...
- 部分完成：`ProviderRepository`、`ConversationRepository` 与 `ConversationRequestContextPreparer` 已落地；普通聊天上下文资格、知识生命周期、窗口和摘要准备已经迁出 ViewModel，网络发送状态与其他运行编排仍需继续迁出。
...
- 部分完成：普通聊天上下文准备已迁出；继续迁出 ViewModel 中的网络发送状态和其他运行编排，使其只负责 UI 状态编排。
...
| P0 | Room、Repository、迁移测试和导出 | Room/Repository、普通聊天上下文 Preparer、Schema 导出、v4→v29、event metadata、Memory/Knowledge FTS、Tool Ledger、Agent Profile、MessagePart、知识引用审计、进程退出观察和用户 ZIP 备份/恢复已完成 | 保证升级和本地数据可恢复 |
