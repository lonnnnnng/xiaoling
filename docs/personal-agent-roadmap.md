...
生产 Workflow 继续精确为 `device.snapshot / device.open_app / device.back / device.home / device.tap_ref / device.type_text`。下一阶段先接通 Registry 完成态的纯内存 evidence，再只在 Redmi 对限定 App 做真实滚动验收；Result codec、Room、答案级 UI 与生产工具集合在此之前继续不变。后台设备自动化、精确定时、Foreground Service、MCP、日历/通知、远程 Channel、多 Agent 和本地模型继续后置。
...
生产 Workflow 仍精确为 `device.snapshot / device.open_app / device.back / device.home / device.tap_ref / device.type_text`。第 123 阶段随后完成 Controller/Registry 执行期 opaque/HMAC evidence seam，并替代 generation-only 成功判断；在 Registry 完成态交接和 Redmi 真实验收完成前，Result codec、Room、答案级 UI 与生产工具集合继续不变。后台设备自动化、精确定时、Foreground Service、MCP、日历/通知、远程 Channel、多 Agent 和本地模型继续后置。
...
小灵 `v0.1.14` 已具备可执行应用内任务的最小个人 Agent：普通聊天与 `/agent` 分流，Runtime 可取消、可限步、可确认、可验证并记录 Run、Step、Approval、Event 和 Memory；Agent Profile v1 已分离身份与能力，Room v33 已让结构化消息、知识引用、Embedding/Shadow、后台停止原因和独立进程退出观察持久化。长期记忆、声明式 Skill、1 至 8 步 Workflow、WorkManager 非精确定时、本地知识库、答案级引用 UI，以及设备 Agent 观察与有限动作层均已交付。完整限定设备工具集开放给前台直接 `/agent`；前台手动 Workflow 当前精确开放同一 Agent Run 内的 `device.snapshot / device.open_app / device.back / device.home / device.tap_ref / device.type_text`，其中打开应用、点击与输入逐动作审批，`open_app` 的唯一包名经过三层白名单并与动作后包名绑定，`back / home` 为空参数、零审批 SAFE 导航。所有动作都要求当前观察和动作后 Executor/typed 验证，`home` 还必须匹配系统动态解析的 launcher。文本输入的跨入口持久路径只保存指纹与长度；Workflow 已能投影 `open_app / back / home / tap_ref / type_text` 的白名单成功证据和稳定失败状态。首批只对小灵、系统计算器、时钟、设置和桌面完成 Redmi 验收，不承诺任意 App；仅 `swipe` 尚未进入 Workflow，全部后台设备自动化、坐标与截图继续关闭。通用执行恢复矩阵已交付提交状态未知分类、用户确认的受控关联新 Run、已验证控制面收尾、两类原子失败结算、持久停止栅栏、Worker 所有权隔离和单调执行预算；旧 Run 与旧副作用事实保持不变，不恢复旧 Executor、模型协程或 Workflow 后续步骤。第 59 阶段约 229.416 秒复合 SAFE 后台成功样本仍未形成自然 LMK 或 Foreground Service 引入依据。第 108 至 121 阶段依次完成 Workflow 只读观察、答案级证据、本地判定、真实双 Run、动作安全契约、`tap_ref`、`type_text`、SAFE `back / home` 与逐包审批 `open_app`；相关性生产拒绝与 answerability enforcement 继续关闭，Shadow 只在真正分隔窗口低频并行观察。
...
- AccessibilityService 观察与有限动作层已经交付；前台手动 Workflow 已精确开放 `snapshot / open_app / back / home / tap_ref / type_text`，但 `swipe`、后台设备执行、坐标/截图兜底和任意 App 通用能力仍未开放。
...
- Workflow 待完成：`device.swipe`。第 122 阶段冻结纯策略契约，第 123 阶段已完成 Controller/Registry 执行期 HMAC evidence seam，并以同窗内容变化与共同匿名锚点方向主位移替代 generation-only 验证；当前剩余 Registry 完成态纯内存交接和 Redmi 限定 App 真实滚动验收，完成前生产工具面继续关闭。第 121 阶段 `open_app` 以聚焦 JVM `95/95`、Debug/AndroidTest APK、Redmi Compose/Room 单项、真实逐包审批 tracer 和文档 corpus 首轮 `OK (1 test)` 完成生产闭环；冻结 corpus 复验同样通过。
...
| P2 | Accessibility 设备工具 | 观察、有限动作、审批、操作后验证和少量指定 App Redmi E2E 已完成；前台手动 Workflow 精确开放 `device.snapshot / device.open_app / device.back / device.home / device.tap_ref / device.type_text`，`swipe`、后台与任意 App 继续关闭 | 扩展到真正移动端执行，风险较高 |
