...
- 发布 `v0.1.18`（`versionCode 19`、Room v36），移除应用内第二张启动图，并启用 Release 资源收缩；`assembleRelease`、签名、`zipalign` 和 SHA-256 资产校验均已完成。
- 本阶段没有重复运行 JVM、Lint、Debug/AndroidTest 或 Redmi instrumentation，发布边界和第 262 阶段已验收的通知保存为笔记能力均未改变。
- 当前主线以个人 Agent 真实任务闭环为第一优先级，遵守“自然语言目标 → 用户确认 → 最小能力面 → 可验证结果 → 当前权威事实查看”的统一契约。

### 后续路线

1. 冻结一条新的高频个人 Agent 任务，先完成可体验的前台闭环，再决定是否扩展能力面。
2. 设备 Agent 继续只在 Redmi 前台、少量指定 App 上推进，扩大已验收覆盖但不承诺任意 App。
3. 在设备能力扩张前，补齐通用执行恢复、长任务和网络/进程异常下的可靠性，保持旧 Run 不变并通过关联新 Run 重试。
4. 精确定时和 Foreground Service 仅依据真实任务耗时、系统停止证据和用户场景决定，不预先引入。
5. MCP、日历/通知扩展动作、远程 Channel、多 Agent、本地模型和云同步继续后置。
6. 按分级验证执行：小阶段做受影响的局部验证，里程碑或正式发版前再做完整矩阵；不让重复测试替代新功能交付。

## 第 262 阶段：通知保存为本地笔记真实竖屏闭环（完成）

- 在第 260/261 阶段的当前通知详情之上，新增“保存为笔记”入口。点击时重新读取当前 NotificationListener，只有非敏感且存在标题或正文的通知才生成草稿；通知 ID、包名、发布时间和正文摘要身份只驻留短生命周期内存，不写 Room 通知历史。
- 草稿复用既有 `notes.create` Agent 链路，明确把标题、正文和包名等字段当作外部不可信数据；不会自动发送、创建 Run/Workflow、批准写入或把通知包名解释为设备目标 App。通知权限撤销、服务断连、通知消失、空内容和敏感内容均 fail-closed。
- 策略单测、定向 Kotlin 编译和 Debug/AndroidTest APK 已通过。Redmi `wsvwypiz7xwslvl7 / begonia / Android 16` 真实 `gpt-5.6-luna` 在竖屏完成通知读取、草稿转换、可见审批和笔记写入；最终 `OK (1 test)`（`57.861s`），`notes.create` 为 `APPROVED / PASSED / COMMITTED`。
- 当前 Room 与 Activity 重建后的笔记详情弹窗使用同一稳定 ID、正文和 revision；弹窗独立控件和 ViewModel 身份均被验证，不能仅用聊天中的正文匹配代替导航成功。临时笔记/Profile/会话/通知/channel 清理，旧 Run digest 不变，成功 Run 审计保留。
- 阶段收尾已完成：六份长期文档同步，最终文档 corpus 单项 `1/1`（`3.485s`），测试包卸载、主应用及用户数据保留，Redmi 已交还其他任务；本阶段纳入 Git 提交推送，不创建 Release。

下一阶段：按第 263 阶段路线冻结新的单一高频个人 Agent 任务。横屏会话区过矮保留为独立 UI 待办；通知点击/回复/清除、后台读取和 Room 通知历史继续后置。
...
- `notifications.get` 原样使用 list 返回的稳定通知 ID；真实 Tool Ledger 与消息 Tool part 一致，Activity 重建后“查看通知”从当前 NotificationListener 重新读取并显示测试 marker。
- 移除通知后再次进入详情显示“当前通知已消失或不可读取”，证明答案入口不回放历史快照；临时通知、channel、Profile/会话精确清理，成功 Run 审计保留。
- 最终 Redmi 单项为 `OK (1 test)`、`29.049s`；未使用模拟器，未运行完整 JVM、Lint、Release 或全量 instrumentation。

下一阶段：回到个人 Agent 主线，重新冻结一条新的单一高频任务，继续遵循“自然语言目标 -> 最小能力面 -> 可验证结果 -> 当前权威事实查看”，通知动作、后台读取和 Room 历史镜像继续后置。
...
- 本阶段只增加只读导航，不开放点击、回复、清除、PendingIntent/RemoteInput、第三方 App 跳转、Room 历史或后台 Agent。聚焦 JVM/AndroidTest Kotlin 编译已通过；真实 Provider 门禁由第 260 阶段完成。

第 260 阶段已完成 `notifications.list -> notifications.get -> 查看通知` 及通知移除后的入口拒绝；旧 Run 保持不变。
...
- 新增仅前台 `DIRECT`、零审批 `SAFE` 的 `notifications.list / notifications.get` 及独立 `notification-overview` Skill。详情只接受同一 Run 最近一次列表返回的稳定 `notification-<SHA-256>` ID；新 Run、通知消失、授权撤销或服务断开全部 fail-closed。
- 监听状态只驻留当前进程，不写 Room，不把原始系统 notification key、PendingIntent、action、RemoteInput 或通知对象交给模型。投影只包含应用名、包名、发布时间和最多 500 字符的规范文本；私密/消息/来电通知以及疑似验证码、密码、口令、密钥、API Key 或 token 内容整体隐藏。
...
第 259 阶段已完成答案级“查看通知”生产入口；第 260 阶段已完成 Redmi 真实 Provider 的自然语言 `notifications.list -> notifications.get -> 查看通知` 验收，并验证通知移除后的拒绝。旧 Run 保持不变，不扩展通知动作。
...
该阶段下一步仍从个人 Agent 主线选择新的窄任务，继续要求用户明确意图、最小 Profile、可验证副作用和权威事实回读；当时不横向扩展全天提醒、重复、多提醒、参与人、联系人写入、通知读取或后台设备动作。通知前台只读观察现已由第 258 阶段完成，通知动作与后台访问仍关闭。
...
该后续工作已由第 248 阶段完成：真实自然语言规划、逐次审批、答案级系统日历查看和清理均已闭环；全天/重复/多提醒/参与人、联系人写入和后台设备动作仍未扩展。该阶段当时后置的通知前台只读观察已由第 258 阶段完成，通知动作与后台访问仍关闭。

## 第 246 阶段：可信联系人答案级系统详情导航（完成）

- 可信 `contacts.get / PASSED` 单一详情结果显示“查看联系人”入口；普通文本、失败结果、伪造/重复 ID 和参数漂移不能产生入口。
- 点击时二次读取当前 Contacts Provider，核对权限、contact ID 与 `lookupKey`，再用 `ACTION_VIEW` 跳转系统权威联系人详情；删除/合并、撤权、Provider 异常、lookupKey 缺失和无处理 Activity 均 fail-closed。
- `lookupKey` 仅作为短生命周期 Provider 导航凭据，不进入工具结果、答案、日志或模型上下文；答案仍只投影最小联系人字段。
- 聚焦 JVM `7/7`、Debug/AndroidTest APK 和仅 Redmi 正向详情/删除竞态 `2/2`、撤权 `1/1`、最终文档 corpus `1/1` 通过。Room v36、Workflow、后台和联系人写入边界不变。

该阶段下一步仍从个人 Agent 主线选择新的窄任务，继续坚持“自然语言目标 -> 可验证结果 -> 当前权威事实查看”；当时不扩展联系人写入/发送、通知读取、后台设备动作、TTS 或多 Agent 生态。通知前台只读观察现已由第 258 阶段完成，通知动作与后台访问仍关闭。
...
第 245 阶段把联系人域接入了“自然语言目标 -> 稳定身份 -> 当前权威详情”的前台只读主链；第 246 阶段随后完成“查看联系人”，第 255 阶段完成可见审批后的系统拨号页预填。联系人写入、直接呼叫和短信/邮件继续后置；通知前台只读观察已由第 258 阶段完成，通知动作、远程 Channel、多 Agent 和本地模型仍后置。
