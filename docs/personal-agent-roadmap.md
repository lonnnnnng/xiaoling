...
- 恢复边界策略已增加：识别可保留的 `WAITING_APPROVAL` Run 与必须安全重启的执行/验证中 Run；真正接入启动协调器仍待完成。
- 启动协调器已接入：待审批 Run 和审批卡片可从 Room 重建；批准后 Runtime 从原审批步骤继续执行工具、验证和总结并写回同一 Run，执行/验证中 Run 仍必须安全重启。
- 启动协调器已接入：待审批 Run 和审批卡片可从 Room 重建；批准后 Runtime 从原审批步骤继续执行工具、验证和总结并写回同一 Run，执行/验证中 Run 仍必须安全重启。

### 主要缺口

- 当前重试采用安全重新运行而不是原地续跑：旧 Run 保持不变，新 Run 关联 `retryOfRunId` 并重新走模型规划、工具审批和验证；`WAITING_APPROVAL` 的原地恢复已接入，执行/验证中 Run 仍不恢复旧执行栈。
- 恢复策略已明确 `WAITING_APPROVAL` 的原地恢复边界；进程重建后真正恢复旧协程、模型调用、工具执行和验证栈仍未交付。
- `WAITING_APPROVAL` 的原地恢复已接入；执行/验证中 Run 的旧协程、工具执行栈和验证栈仍不恢复。
...
- 已有结构化长期记忆表、`memory.search / memory.remember`、FTS 检索、管理 UI、候选确认、敏感过滤、删除撤销、去重和冲突处理；记忆过期和引用审计还没有完成。
...
1. 设计进程重建后的原地断点恢复，明确哪些步骤可续跑、哪些必须创建新 Run。
   当前已完成启动协调器、待审批 UI、批准后的原 Run 执行入口、恢复执行失败的安全重试测试和 Room 集成测试；下一步只剩不清理 Keystore 的真实进程重建验收。
2. 已完成跨进程删除撤销；后续后台任务必须复用原子快照与 Room 状态核对边界。
3. 内置 Skill 按需选取已完成；下一步推进多步 Agent，再补本地 Skill 导入和后台任务。
