...
98. 下一阶段：继续积累更多用户显式开启的真实前台样本，观察成本和自然失败分布；样本不足前不增加 Room Store，不开启 enforcement。
99. 仍后置：设备工具进入 Workflow/后台自动化、精确定时、Foreground Service、MCP、日历/通知、远程 Channel、多 Agent 和本地模型。
