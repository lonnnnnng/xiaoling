...
- `settleScheduledWorkflowRun()` 在同一 Room transaction 中重新读取 Task、关联 Workflow Run 与停止栅栏。`STOP_REQUESTED` 固定收敛为取消；若旧版 fallback、重入或进程终止已经让 Workflow 先进入终态，则该持久终态映射到 Task，迟到 Worker 不能把 Task 写成相反状态。结算结果与本轮 outcome 不一致时不追加本轮会话消息，通知也读取持久状态，避免取消/失败链显示成功。
