...
- `v0.1.18`（`versionCode 19`、Room v36）已发布。应用内第二张启动图已移除，Release 资源收缩已启用，正式 APK 已完成构建、签名、`zipalign` 与 SHA-256 资产校验。
- 本阶段按快速迭代约束只做发布资产校验，没有重复运行完整 JVM、Lint、Debug/AndroidTest 或 Redmi instrumentation；生产 Tool、Skill、权限、Workflow、设备动作和后台边界不变。
- 后续实现以个人 Agent 的单一高频真实任务为主线，沿用最小 Profile、用户确认、typed verification、当前权威事实回读和旧 Run 不变的恢复契约。设备 Agent 仍限 Redmi 前台少量指定 App；可靠性优先于扩大任意 App 或后台自动化。
...
3. 补齐通用执行恢复、长任务以及网络/进程异常处理；不原地重放未知副作用，必要时创建关联新 Run。
