...
- Text/Reasoning/Tool 消息 parts：Room 独立保存 part ID、消息内顺序、类型、正文、供应商摘要来源与 item 身份、工具参数、结果、成功状态、验证状态和记忆引用。Reasoning 只接受 Responses `reasoning.summary[].summary_text`，不读取或展示原始 `reasoning_text/reasoning_content`；默认折叠并标明供应商来源，debug 响应与 SSE 日志也必须脱敏原始推理。Agent Tool part 必须由应用可信上下文投影，Reasoning 不进入 `VerifiedAgentContext`、不能生成或替代 Tool。前台快照保存只能增量 upsert，用户删除必须显式传递会话 ID，以保留并发后台 Workflow 新建或追加的消息证据。
...
- Room v23 本地保存 Provider、Agent Profile、会话、消息及 MessagePart、Agent Run、审批、独立工具调用/结果、笔记、长期记忆、候选记忆、记忆操作映射、Skill、Workflow、WorkflowStepDefinition、WorkflowSchedule 和 ScheduledTask Ledger；RunEvent 使用独立 typed metadata 保存时间线事实，长期记忆使用 FTS4 索引。v21→v22 新建 `message_parts` 并只回填 Text，v22→v23 为 Reasoning 增加可空来源、供应商 item ID 与 summary index；升级不从旧正文、`verifiedAgentContext` 或工具记录猜造 Reasoning。v4→v23、v21→v22、v22→v23 和全新 v23 建库已有 Schema 与迁移测试保护。
