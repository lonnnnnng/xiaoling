...
- 恢复边界已明确：`WAITING_APPROVAL` 且只有待处理审批、尚未进入工具执行/验证的 Run 可以保留原 Run 等待用户决定；已经进入工具执行或验证的 Run 必须创建新 Run 安全重新运行，旧 Run 保持不变。真正的执行栈重建仍未交付。
- 应用重启后会把可恢复审批重新显示到对应会话；由于旧协程不存在，用户继续操作时关闭旧 Run 并创建带 `retryOfRunId` 的新 Run，执行/验证中的 Run 仍直接安全收敛。
- 应用重启后可恢复的审批批准后，会从持久化审批步骤继续执行同一 Run，并重新写入工具结果、验证和最终总结；执行/验证中的 Run 仍直接安全收敛并要求新 Run 重试。
