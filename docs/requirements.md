...
- 本地知识库与管理 UI：第一版只导入 TXT、Markdown、JSON 和 CSV 的严格 UTF-8 文本，最大 64 MB / 1600 万 UTF-16 字符，统一移除 BOM、规范换行、拒绝空白文档与二进制空字符，并对规范全文计算 SHA-256。文档身份与 revision 分离；确定性分块优先在段落边界结束，默认每块 1600 字符、固定最多 200 字符重叠，offset 指向 Room 内规范全文且不得切断 UTF-16 代理对。Room v26 保存全文、chunks、FTS4 索引和每次检索审计；检索同时保留中文与字面通配符安全的 `LIKE` 兜底。设置页必须支持 SAF 导入、轻量摘要列表、详情、启停、替换、删除和显式检索预览；列表不得读取完整正文，详情只从 SQLite 投影并安全截取最多 4,000 个 UTF-16 单元，不得切断代理对，截断时必须明确标记。替换必须在同一事务内递增 revision、更新 parser/hash、删除旧 chunks/FTS 并生成包含文档身份和新 revision 的 chunk ID；失败整笔回滚。禁用、替换或删除开始时必须取消在途检索并清空旧引用，旧详情或刷新请求不得覆盖用户的新选择；已提交变更后的刷新失败必须与提交失败分开呈现。禁用或删除后立即退出检索，旧 chunk 引用必须失效。当前阶段不包含 Agent 工具、模型上下文注入或答案引用呈现。
...
- Room v26 本地保存 Provider、Agent Profile、会话、消息及 MessagePart、Agent Run、审批、独立工具调用/结果、笔记、长期记忆、候选记忆、记忆操作映射、Skill、Workflow、WorkflowStepDefinition、WorkflowSchedule、ScheduledTask，以及知识文档全文、chunks、FTS 和检索审计；RunEvent 使用独立 typed metadata 保存时间线事实，长期记忆和知识 chunks 分别使用 FTS4 索引。v21→v22 新建 `message_parts` 并只回填 Text，v22→v23 为 Reasoning 增加可空来源身份，v23→v24 为 Image 增加附件列，v24→v25 为 Document 增加可空提取文本、页数和 detail，v25→v26 只创建空知识库表；升级不从旧正文、URI、`verifiedAgentContext` 或工具记录猜造 Reasoning/Image/Document/Knowledge。v4→v26、各增量迁移和全新 v26 建库已有 Schema 与迁移测试保护。
...
- 审批恢复和已提交结果恢复必须使用原 Run 的 Agent Profile 快照，而不是当前选中的 Profile。缺少 Profile 审计的历史 Run 保留旧兼容路径；新 Run 出现重复、损坏、引用未注册工具或 Skill 越权的 Profile 审计时必须拒绝恢复，不能回退当前 Profile 扩大能力。
- 应用重启后可恢复的首步审批批准后，会从持久化审批步骤继续执行同一 Run，并重新写入工具结果、验证、后续多步规划和最终总结；第一步已经执行后若在第二步审批或执行/验证阶段中断，仍直接安全收敛并要求新 Run 重试。

当前仍未交付 Agent 知识检索工具、模型上下文引用注入、答案引用呈现、Embedding，以及执行/验证中 Agent Run 的通用原地恢复、并行工具调用、后台 Workflow 执行栈的断点续跑、精确定时、Foreground Service 和手机自动化；本地知识库的 Room v26 数据、确定性分块、FTS/中文兜底、替换失效、检索审计和管理 UI 已交付。执行/验证中进程终止与 Android 权限运行中撤销已经完成确定性故障注入。执行回执 contract、证据判定 module，以及 `notes.create`、`memory.remember` 的验证阶段恢复已交付。`memory.remember` 的 Room operation ledger 绑定 ToolCall、原始载荷哈希、memory ID 和结果快照哈希；Registry 仅在当前记忆仍等于提交快照、启用且未过期时进入 `COMMITTED_TOOL_VERIFICATION`。v18 历史 operation 因缺少结果快照继续要求新 Run。多步骤 Workflow 与非精确调度均已交付并完成真机验收；当前真实后台三步骤耗时约 31 秒，尚无引入 Foreground Service 的必要。数据库恢复已交付，但跨设备 Provider 密文恢复仍受 Android Keystore 限制。
