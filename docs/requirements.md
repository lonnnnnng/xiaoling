...
- Text/Reasoning/Image/Tool 消息 parts：Room 独立保存 part ID、消息内顺序、类型、正文、供应商摘要来源与 item 身份、图片 MIME/文件名/BLOB/detail、工具参数、结果、成功状态、验证状态和记忆引用。Image 仅允许 USER 来源，系统选择器单次接收 PNG/JPEG/WEBP，应用以 8 MB 有界读取并校验 MIME、签名和可解码性；Responses 映射为 `input_image` Data URL，Chat Completions 与 `/agent` 明确拒绝。Reasoning 只接受 Responses `reasoning.summary[].summary_text`，不读取或展示原始 `reasoning_text/reasoning_content/encrypted_content`；图片 Base64 与原始/加密推理在 debug 日志中必须脱敏。Agent Tool part 必须由应用可信上下文投影，Image/Reasoning 均不进入 `VerifiedAgentContext`、不能生成或替代 Tool。图片 BLOB 只为当前会话加载，切换会话必须在完整 parts 就绪后原子更新界面；轻量快照保存不得清空未加载 BLOB，网络请求必须等待用户消息与图片事务提交。前台快照保存只能增量 upsert，用户删除必须显式传递会话 ID，并在事务前过滤删除集合，以保留并发后台 Workflow 证据且防止陈旧快照复活已删会话。
...
- Room v24 本地保存 Provider、Agent Profile、会话、消息及 MessagePart、Agent Run、审批、独立工具调用/结果、笔记、长期记忆、候选记忆、记忆操作映射、Skill、Workflow、WorkflowStepDefinition、WorkflowSchedule 和 ScheduledTask Ledger；RunEvent 使用独立 typed metadata 保存时间线事实，长期记忆使用 FTS4 索引。v21→v22 新建 `message_parts` 并只回填 Text，v22→v23 为 Reasoning 增加可空来源身份，v23→v24 为 Image 增加可空 MIME、文件名、BLOB 和 detail；升级不从旧正文、URI、`verifiedAgentContext` 或工具记录猜造 Reasoning/Image。v4→v24、各 MessagePart 增量迁移和全新 v24 建库已有 Schema 与迁移测试保护。
