...
- 后续重试必须创建带 `retryOfRunId` 的独立新 Run，新 Run 不得继承旧审批、步骤或 Tool Ledger；旧 Run 的事件和事实只读保留。
- 该边界不允许恢复旧模型协程、原地重放未知副作用或把迟到 UI 决定解释为新的授权。第266阶段还需继续覆盖 `QUEUED` 进程对账和长任务中断分类。

## 系统计算器前台计算任务（第 265 阶段，完成）

- 个人 Agent 冻结一条指定 App 窄任务：系统计算器内清空一次当前算式，按 7、乘法运算符、8、等号完成计算，并从当前屏幕读取结果。
- 设备动作拆成独立步骤；算式显示区不作为输入框，不能用 `device.type_text` 代替逐键操作。每个动作需要同 Run 新鲜观察、短生命周期节点引用、风险审批和操作后验证。
- 目标包固定为 `com.android.calculator2`，只在 Redmi `wsvwypiz7xwslvl7` 真机验收；完成标准还要独立读取计算器 `formula/result` 控件的 `56`。
- 已通过真实 Provider 计划只读门禁与 Redmi 前台动作验收：6 步计划、5 次逐动作审批、每步重新观察/验证，最终结果为 `56`，小灵恢复后显示 `7/7` 步骤完成。

## 第 266 阶段可靠性约束（进行中）

- 设备 snapshot、window generation 与短生命周期 ref 失效时，必须在 Executor 前 fail-closed，写入结构化恢复失败，不自动刷新后重放旧动作。
- 模型规划失败必须保留失败分类、执行预算和 typed `retryDisposition`；无工具副作用的瞬态网络失败可以创建独立新 Run，已有工具事实或证据不足必须重新确认，鉴权/地址/模型错误先修配置。
- 模型总结失败不能改判已验证工具事实；必须使用本地可信兜底并记录 `LOCAL_FALLBACK_COMPLETED`。旧 Run、旧审批和 Tool Ledger 保持不变。
- 第 266 阶段下一步继续处理进程异常、审批等待和长任务中断；不提前引入后台设备自动化、精确定时或 Foreground Service。


## 一次性提醒改期（第 264 阶段，完成）

- 用户可明确要求修改尚未开始的一次性提醒。工具只允许前台 DIRECT、当前 Profile 显式授权与逐次审批，旧 Skill/Profile 不自动获得新能力。
- 必须从 `tasks.list` 的唯一精确名称进入 `tasks.inspect`，取得当前原时间和计划指纹，再调用 `tasks.reschedule(name, expected_planned_at, planned_at, time_zone, expected_schedule_token)`。审批必须完整显示任务、原时间、新时间和时区，不得省略成两行参数摘要。
- 两个时间都使用带偏移的 ISO-8601，偏移必须与 IANA 时区一致。提交时新时间距当前至少 1 分钟且最多 7 天；没有有效 WorkRequest、同名、多活动实例、周期规则、停用或运行中的任务均不可改期。
- 入队新计划前不取消原计划；系统入队成功后在事务内重新比较原时间、稳定身份和调度版本，Worker 抢占或审批漂移时拒绝。成功只替换旧待执行实例，保留其原时间和取消记录，不改写旧 Run、历史步骤或已执行实例。
- 新系统工作项与当前 Room 时间回读一致才可报告成功，并从可信结果提供“查看任务”。未知提交保持 `RESTART_REQUIRED + DENY`，不得自动重放；不增加周期规则编辑、后台改期、精确闹钟或 Foreground Service。
- 第 264 阶段真实前台闭环已收口，下一项为第 265 阶段的一个指定 App Redmi 前台任务，第 266 阶段针对实际缺口补可靠性；里程碑再做完整矩阵，发版单独授权。
- 已通过 JVM `180/180`、Debug/AndroidTest APK、Redmi 存储测试 `6/6`，以及真实 `gpt-5.6-luna` 前台单项 `1/1`（`39.449s`）。自然语言、可见审批、真实 WorkManager 旧取消/新入队、旧 Run 不变、重建后从改期结果查看当前任务均已验证；不包含改期后的到点执行或精确调度验收。
...
- 精确定时和 Foreground Service 只根据真实耗时与系统停止证据评估；MCP、日历/通知扩展动作、远程 Channel、多 Agent、本地模型和云同步继续后置。
- 验证采用分级门禁：小阶段执行受影响的局部检查，里程碑或正式发版前执行完整矩阵；测试不能替代功能交付。
...
- Room v36 本地保存 Provider、Agent Profile、会话、消息及 MessagePart、Agent Run、审批、独立工具调用/结果、笔记、长期记忆、候选记忆、记忆操作映射、Skill、Workflow、WorkflowStepDefinition、WorkflowSchedule、ScheduledTask、独立进程退出观察、匿名 answerability Shadow 观测，以及知识文档全文、chunks、FTS、Embedding 向量和检索审计；RunEvent 使用独立 typed metadata 保存时间线事实。v25→v26 只创建空知识库表，v26→v27 增加知识引用 JSON，v27→v28 增加后台 Worker 停止原因，v28→v29 创建退出观察表，v29→v30 增加按 Provider/模型隔离的向量表与检索身份/状态，v30→v31 增加可空的 top1、top2、margin 和有效候选数，v31→v32 增加可空的候选均值、总体标准差和 top1 z-score，v32→v33 创建空的匿名 Shadow 账本，v33→v34 增加 Workflow 目标包，v34→v35 增加目标级完成标准，v35→v36 增加笔记 revision 与编辑操作账本；升级不从旧正文、URI、`verifiedAgentContext`、工具记录、旧错误文案、当前向量、历史阶段合计或时间邻近关系猜造知识引用、Embedding 分数、相对分布、Shadow 记录、系统停止原因或 Task/Run 归因。v4→v36、各增量迁移和全新 v36 建库已有 Schema 与迁移测试保护。
