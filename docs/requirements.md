...
- 本地知识库数据基础：第一版只导入 TXT、Markdown、JSON 和 CSV 的严格 UTF-8 文本，最大 64 MB / 1600 万 UTF-16 字符，统一移除 BOM、规范换行、拒绝空白文档与二进制空字符，并对规范全文计算 SHA-256。文档身份与 revision 分离；确定性分块优先在段落边界结束，默认每块 1600 字符、固定最多 200 字符重叠，offset 指向 Room 内规范全文且不得切断 UTF-16 代理对。Room v26 保存全文、chunks、FTS4 索引和每次检索审计；检索同时保留中文与字面通配符安全的 `LIKE` 兜底。替换必须在同一事务内递增 revision、更新 parser/hash、删除旧 chunks/FTS 并生成包含文档身份和新 revision 的 chunk ID；失败整笔回滚。禁用或删除后立即退出检索，旧 chunk 引用必须失效。当前阶段不包含知识库管理 UI、Agent 工具、模型上下文注入或答案引用呈现。
