...
- Text/Tool 消息 parts：Room 独立保存 part ID、消息内顺序、类型、文本、工具参数、结果、成功状态、验证状态和记忆引用。普通流式与历史文本继续兼容 `messages.text`；Agent Tool part 必须由应用可信上下文投影并在同一消息气泡中展示，普通 assistant 文本不能生成 Tool part。前台快照保存只能增量 upsert，用户删除必须显式传递会话 ID，不能把快照缺失推断为删除，以保留并发后台 Workflow 新建或追加的消息证据。
...
- Room v22 本地保存 Provider、Agent Profile、会话、消息及 MessagePart、Agent Run、审批、独立工具调用/结果、笔记、长期记忆、候选记忆、记忆操作映射、Skill、Workflow、WorkflowStepDefinition、WorkflowSchedule 和 ScheduledTask Ledger；RunEvent 使用独立 typed metadata 保存时间线事实，长期记忆使用 FTS4 索引。v19→v20 新建空工具账本，v20→v21 新建空 Agent Profile 表，v21→v22 新建 `message_parts` 并为每条旧消息回填唯一 Text part；迁移不从旧 `verifiedAgentContext` 补造 Tool part。v4→v22、v19→v20、v20→v21、v21→v22 和全新 v22 建库已有 Schema 与迁移测试保护。
