...
- Room v21 本地保存 Provider、Agent Profile、会话、消息、Agent Run、审批、独立工具调用/结果、笔记、长期记忆、候选记忆、记忆操作映射、Skill、Workflow、WorkflowStepDefinition、WorkflowSchedule 和 ScheduledTask Ledger；RunEvent 使用独立 typed metadata 保存时间线事实，长期记忆使用 FTS4 索引，旧 SharedPreferences 数据首次启动时迁入。v16→v17 为笔记增加可空唯一幂等键；v17→v18 新建 `agent_memory_operations`；v18→v19 为记忆 operation 增加可空结果快照哈希；v19→v20 新建空的 `agent_tool_calls / agent_tool_results`；v20→v21 新建空的 `agent_profiles`，不从全局 Provider 偏好伪造 Agent 身份。v4→v21、v17→v18、v18→v19、v19→v20、v20→v21 和全新 v21 建库已有 Schema 与迁移测试保护。
