...
当前仍未交付进程重建后的原地继续执行、记忆过期与本轮引用审计、跨进程删除撤销、Skill、后台工作流和手机自动化。数据库恢复已交付，但跨设备 Provider 密文恢复仍受 Android Keystore 限制。现有 Agent “恢复”采用安全重新运行：创建新 Run、使用当前 Provider/模型、重新审批写入工具，不复用旧执行栈。
