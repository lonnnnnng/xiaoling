...
当前仍未交付执行/验证中 Run 的原地恢复、多步 Agent、本地 Skill 导入与管理、后台工作流和手机自动化；待审批 Run 原地继续、内置 Skill 按需选取、跨进程记忆删除撤销、记忆过期、时间衰减、实际引用审计与单次召回关闭已交付。数据库恢复已交付，但跨设备 Provider 密文恢复仍受 Android Keystore 限制。
