...
- 通用执行恢复与长任务可靠性优先于扩大设备能力；旧 Run 保持不变，提交状态未知时不得猜测或原地重放，必要时创建关联新 Run。
- 精确定时、Foreground Service、MCP、日历/通知扩展动作、远程 Channel、多 Agent、本地模型和云同步继续后置，只有真实耗时和系统证据支持时才立项。
