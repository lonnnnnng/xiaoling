...
当前状态：Room v16 Schema、迁移测试、RunEvent typed metadata、完整 Tool Registry 契约、AgentRuntime、审批/验证、确定性测试、任务中心、安全重新运行、长期记忆治理和一次性/Daily/Weekly Workflow 调度已完成；执行/验证中断采用旧 Run/活动 Step 一致取消和关联新 Run 重试，原地续跑只有在持久化执行回执与幂等副作用证明可用后才重新评估。独立 ToolCall/ToolResult 表、完整消息 parts 和 AgentProfile 仍待完成。
