...
下一版不应跳到 MCP 或“任意控制手机”。累计执行预算、步骤落库后的 Workflow 启动对账、超时边界、需确认聚合、结构化安全处置、Doze/受控内存/无压力对照、当前进程内 Worker 所有权隔离、后台 `RUNNING` 实例的用户可见停止，以及 `STOP_REQUESTED` 持久化停止栅栏均已完成。Workflow 仍活动时，停止意图先于 WorkManager 取消落库，系统取消与即时 fallback 同时失败后仍可由启动恢复继续收敛；Workflow 已先终态时则直接对账半结算 Task，不伪造停止覆盖历史事实。迟到 Worker 不能把 Workflow/Task 写成互相矛盾的状态，周期下一实例也不会提前物化。Redmi 已有约 62.2 秒八步 SAFE 全成功样本和约 32.6 秒真实停止样本；设备支持 LMK 原因报告，但最新 6 条退出均为 instrumentation `FORCE STOP`，`REASON_LOW_MEMORY=0`，仍未取得 Android 自主 LMK。第 52 阶段已为启动恢复和确认弹窗增加 Ledger/Event canonical SHA-256 指纹；即使证据分类仍为 COMMIT_UNKNOWN，新增调用、参数/回执替换或验证事件漂移也会拒绝旧确认并升级为 EVIDENCE_INCOMPLETE。第 53 阶段已用真实 Runtime seam 验证 ToolResult、预算快照和 tool.verify 三个提交边界：预算快照缺失固定拒绝原地恢复，验证事件已落库只补控制面。第 54 阶段已让规划异常先冻结 telemetry/预算，并让总结网络失败使用本地可信回复完成原 Run；Receipt 回读失败保持 typed 错误码与需确认重试。第 55 阶段又把流式断流、HTTP/鉴权/限流、DNS/TLS、响应解析及未知模型异常统一投影为 typed `llm.request.failed` 稳定错误码，任务事件区只展示阶段、分类和脱敏原因。第 56 阶段已完成部分 delta 断流的用户可见收敛，并将失败/取消的部分正文排除出后续上下文。第 57 阶段已让取消收敛先写最新单调预算再写终态，避免后台停止丢失已消耗时间。下一步只验证更长真实后台任务中的预算快照与系统回收组合行为、以及 Android 自主 LMK，再决定是否需要 Foreground Service 或更强恢复机制。设备工具仍不进入 Workflow 或后台自动化；精确定时继续依据真实需求决定，日历/通知、MCP、远程 Channel、多 Agent 和本地模型保持最后推进。
