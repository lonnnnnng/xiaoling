...
实施状态同步至 2026-07-19：本文提出的 AgentProfile v1 已在 Room v21 落地，Text/Tool 消息 parts 已在 Room v22 落地，供应商 Reasoning summary 已在 Room v23 落地；参考项目审计日期仍保持原始取证时间。
...
| 缺口 | 当前影响 |
|---|---|
| AgentProfile 与 Text/Reasoning/Tool parts 已完成 | Agent 身份、模型、自然语言、供应商摘要和工具事实已进入稳定边界；下一步评估 Image/Document |
...
| Text/Reasoning/Tool 已结构化，Image/Document 未完成 | 供应商 summary 可恢复、默认折叠且与 Tool 事实隔离；附件仍没有本地 part 生命周期、文件治理和备份边界 |
...
当前状态：Room v23 Schema、迁移测试、Agent Profile v1、Text/Reasoning/Tool 消息 parts、RunEvent typed metadata、独立 ToolCall/ToolResult Ledger、完整 Tool Registry 契约、AgentRuntime、审批/验证、任务中心、长期记忆治理和 Workflow 调度已完成。Tool part 与可信 Agent 上下文一致时保留稳定数据库 ID，漂移时回退可信投影；Reasoning 只保存供应商 summary，不进入可信 Agent 上下文。旧 SQL 迁移不补造 Tool 或 Reasoning。Image/Document parts 仍待完成，其他执行/验证中断继续采用旧 Run/活动 Step 一致取消和关联新 Run 重试。
...
| 消息 parts | Text/Reasoning/Tool 已完成独立 Room 表、旧 text 回填、供应商 summary 身份、可信 Tool 投影、前后台增量写入、显式删除和同气泡展示；后续评估 Image/Document | 流式文本、折叠推理摘要和工具步骤可在同一消息中恢复；原始思维链与 Agent 工具事实保持隔离 |
