...
第 251 阶段采用成熟 Agent 的“先冻结权威源、用户可见审批、幂等提交、提交后回读与稳定回执恢复”原则。小灵没有把笔记标题或搜索预览直接当作导入依据，而是在同一 Run 内先证明唯一候选，再读取当前 revision 与正文 SHA-256；审批后仍从 Note Store 防漂移回读，知识 Store 用稳定 document ID 防止重复创建。成功只在当前文档/chunks 与回执一致时成立，并复用既有知识引用导航。实现没有复制后台笔记监听、自动摄取、标题匹配覆盖、共享知识同步或旧 Profile 静默扩权；真实模型与屏幕审批作为下一独立验收阶段。
