...
| 长期记忆候选治理闭环已完成，但自动治理仍不完整 | 已有候选确认、敏感过滤、去重/冲突和当前会话删除撤销；缺少过期、跨进程撤销和实际引用审计 |
...
当前状态：记忆表、工具读写、来源审计、FTS4 + 中文兜底检索、管理页、编辑、置顶、启停、候选生成与确认、敏感过滤、去重/冲突和当前会话删除撤销已完成；过期、跨进程撤销和本轮引用审计仍待完成。
