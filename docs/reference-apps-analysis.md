...
第 170 阶段继续采用参考实现中“先定位稳定对象，再回到权威存储读取详情”的原则：`notes.get` 只接受 `notes.list / notes.search` 返回形态的 `note-UUID`，正文来自当前 Store，不从模型文本或历史摘要恢复，并明确标记为本地数据而非工具指令。不存在和 tombstone 不作可观察区分，异常旧正文有 20,000 字符上限；新增独立 SAFE Skill 以避免历史 Profile 被隐式扩权。没有复制任意 App、后台设备控制、远程 Channel 或多 Agent 能力，本阶段按用户要求未执行验证。
