...
## 第274阶段：真实 Provider 计划仍需 fail-closed

成熟 Agent 通常把模型计划当作不可信输入，先经过 schema、能力边界和确认前检查，再进入执行层。小灵本阶段用 Redmi 真实 Provider 验证动态应用候选不会绕过这条链：系统计算器被选为有效目标，但计划步骤缺失时直接拒绝，不把部分计划提交给 Workflow。

- 真实计划确认前没有创建 Run、Workflow 或设备动作；重跑得到完整计划后才报告计划验收成功。
- 该证据只证明动态候选与计划门禁协同，不复制任意 App 控制、后台执行或模型自动批准。

## 第273阶段：能力候选与执行授权分离（已完成）

参考项目普遍把“能力发现/规划”与“工具执行授权”分成两层；小灵本阶段把当前 Launcher 目录的脱敏能力候选注入规划上下文，但仍只取已登记白名单交集。名称和分类帮助模型理解“当前设备有哪些候选”，不构成打开应用、审批或跨包权限。

- 目录候选按能力、名称和包名稳定排序，未登记包不会进入提示；模型不得把名称、包名或分类当作指令，也不得猜测目录外包。
- `device.open_app`、Accessibility、动作后观察、目标级验证和后台边界保持现状；不复制参考项目的任意 App 控制或全包枚举，不申请 `QUERY_ALL_PACKAGES`。
- Redmi `wsvwypiz7xwslvl7` 的真实目录/规划前置定向验收为 `OK (2 tests)`；这只证明候选注入与白名单交集，不证明新的应用页面能力已验收。

## 第272阶段：系统设置搜索的受控跨包边界（已完成）

成熟 Agent 的系统设置搜索通常会由设置宿主把查询交给独立的搜索组件；小灵只登记一条明确的 `com.android.settings.intelligence` 伴随关系，用于当前已确认设置任务的动作前后观察和最终判定。

- 伴随包不是可启动目标，不进入 Launcher 目录、规划交集或 `device.open_app` 白名单。
- 任何其他跨包跳转仍拒绝；Redmi `wsvwypiz7xwslvl7 / begonia` 已通过新鲜快照、两次逐动作审批、后置观察和当前页面回读确认，最终目标级结论为 `VERIFIED`，搜索结果页回读 3 条 Wi-Fi 相关结果。
- 独立前台 Debug 验收使用生产 Workflow 安全策略和 Room 脱敏审批投影；为了避免等待浮层造成节点过期，仅替换 Debug 通道的决定来源，未修改生产审批或跨包策略。

## 第271阶段：规划前置的应用可用性门禁

本阶段把第270阶段的“目录候选”接到规划前置，但仍保持发现层与执行层分离：只有当前 Launcher 目录中出现、且已在 `DeviceActionPolicy.DEFAULT_ALLOWED_PACKAGES` 登记的包，才会进入设备任务规划提示；`PersonalTaskPlanPolicy.parse` 还会对模型返回的目标包做第二次交集校验。Redmi 实际目录 30 项中得到 4 个有效交集包，未登记包没有进入规划集合。使用 `AGENTS.md` 兜底 Provider 的真实计划探针和第265阶段计划模式均通过，确认前没有 Workflow 或设备动作。

- 该做法借鉴成熟 Agent 的“能力/工具先经过策略裁剪，再交给模型编排”，但不复制任意 App 控制或全包枚举。
- 本阶段只补验真实 Provider 规划；审批、动作和目标验证仍不由计划模式代替，有效交集只代表规划候选，不代表页面能力已验证。
- `device.open_app` 最终白名单、Accessibility 后置验证和后台边界保持不变。

## 第270阶段：应用发现与能力候选边界

小灵本阶段采用“发现层与执行层分离”的实现：前台只读目录提供当前用户可启动的 Launcher 应用和有限能力候选，不能因为发现到某个包就自动获得 `device.open_app` 或其他设备动作权限。该边界与参考项目中“工具策略先于能力编排”的方向一致，但没有照搬任意 App 控制或全包枚举。

- 目录入口：`ACTION_MAIN + CATEGORY_LAUNCHER`，不申请 `QUERY_ALL_PACKAGES`，不把隐藏/不可启动包当成可用应用。
- 输出边界：只返回名称、包名和候选分类；版本、签名、权限、安装来源、Provider、账号和应用内数据不进入模型。
- 执行边界：发现结果只供前台直接 Agent 读取；真正操作仍要通过已登记包、用户确认、逐动作审批、Accessibility 观察、操作后重新观察和目标级验证。
- Redmi 证据：`wsvwypiz7xwslvl7 / Redmi Note 8 Pro` 已真实读取 30 个 Launcher 入口，目录未截断且自身入口可见；这只证明发现层契约，不证明候选应用能力已验证。
- 后续评估：只有 Redmi 真实验收证明某个候选应用具备稳定可观察事实后，才把它接入具体个人任务规划；不因分类名称命中就扩大白名单。

第 263 阶段发布收尾后，路线重新收敛到个人 Agent 主线：先冻结单一高频任务并完成“自然语言目标 → 用户确认 → 最小能力面 → 可验证结果 → 当前权威事实查看”，再扩大 Redmi 前台少量指定 App 的已验收任务覆盖；在复杂长链或后台/定时设备自动化前，围绕真实任务补齐剩余中断恢复与长任务可靠性，复用已经交付的恢复基座。精确定时、Foreground Service、MCP、日历/通知扩展动作、远程 Channel、多 Agent、本地模型和云同步继续后置。参考项目的后台自动化、任意 App 控制和开放式扩展只作为取舍输入，不改变小灵当前的最小授权、旧 Run 不变与当前权威事实边界。

当前正式版本为 `v0.1.18`（`versionCode 19`、Room v36）。第 263 阶段只将 Android 12+ 系统 Splash 调整为透明占位图标和同色背景、启用 Release 资源收缩并完成发布资产校验，没有扩展生产 Tool、Skill、权限、Workflow 或后台能力。

第 267 阶段把“应用能力”与“ROM 包名”分开：目标级完成判定和持久化恢复复用已登记的等价包族，允许 AOSP/Google 计算器、时钟实现互相验证，同时继续要求默认白名单、逐包审批和动作后观察。它没有复制任意 App 的模糊匹配，也没有扩大设备控制范围；下一步只对已有白名单包族做 Redmi 真实个人任务验收。
...
第 251 阶段采用成熟 Agent 的“先冻结权威源、用户可见审批、幂等提交、提交后回读与稳定回执恢复”原则。小灵没有把笔记标题或搜索预览直接当作导入依据，而是在同一 Run 内先证明唯一候选，再读取当前 revision 与正文 SHA-256；审批后仍从 Note Store 防漂移回读，知识 Store 用稳定 document ID 防止重复创建。成功只在当前文档/chunks 与回执一致时成立，并复用既有知识引用导航。实现没有复制后台笔记监听、自动摄取、标题匹配覆盖、共享知识同步或旧 Profile 静默扩权；后置的真实模型与屏幕审批已由第 252 阶段完成。
...
第 224 阶段采用成熟 Agent 的“能力可发现但执行必须显式确认”原则：Skill 定义自带的触发示例只成为对话草稿入口，应用壳在导航前按当前 Profile 与工具表重新核对，不把管理页点击升级为模型调用或工具授权。该阶段没有复制参考项目的一键自动运行、后台 Skill 调度、远程触发、跨设备同步或多 Agent 编排。
...
第 203 阶段继续采用参考 Agent 的“执行结果先进入可验证账本，用户查看时回到权威存储”的边界：真实 `memory.remember` 结果只在 Debug 探针中短暂写入专属 Room 会话，重建后投影为唯一可信 Tool part，并以稳定记忆 ID 提供导航；临时会话随后精确清理。该切片没有复制跨会话同步、后台批量记忆治理、任意 App 控制、远程协作或多 Agent 编排，也没有把 Debug 验收入口带入 Release；它证明的是持久化投影，不是完整人工 UI/审批点击自动化。

第 202 阶段把参考 Agent 的“副作用必须经过审批、提交后回权威 Store、答案只携带稳定身份”原则落到真实长期记忆写入。Redmi 真实模型只通过最小 Debug Profile 调用 `memory.remember`，Room 审批、Executor/typed verification、提交回执和当前记忆回读共同决定成功；测试夹具完成后精确清理。没有复制跨设备记忆同步、后台批量治理、远程协作或多 Agent 编排，也没有把 Debug 验收入口带入 Release。

第 201 阶段把参考 Agent 的“副作用完成后只提出稳定资源身份，查看时回权威 Store”原则延伸到长期记忆写入。应用回读验证成功后才把 `memory-UUID` 放入结果和 `memoryIdsUsed`，点击入口再重读当前 Room；没有复制跨设备记忆同步、远程协作、后台批量治理或多 Agent 编排。

第 200 阶段把参考 Agent 的“答案入口只提出稳定身份，打开时回权威存储核对”原则应用到本地笔记详情和编辑回执。`notes.get` 的正文只作为明确标记的本地资料，`notes.update` 只有在应用验证的新 revision 与请求严格衔接时才提出入口；点击仍回到当前 Note Store，不把模型文本或历史回执当作事实。没有复制参考项目的跨设备笔记同步、远程协作、后台批量编辑、任意 App 或多 Agent 编排。

第 199 阶段把“副作用完成回执只提出稳定资源身份，详情继续回权威源核对”的原则应用到日程创建和修改。应用不从模型总结或审批状态猜测事件，而只接受已验证 Tool 结果的固定外壳、同一事件 ID 和版本变化后的新指纹，再由 Calendar Provider 详情页回读；删除结果不制造已不存在资源的入口。没有复制参考项目的日历镜像、后台同步、远程协作或自动授权。

第 198 阶段把参考 Agent 常见的“答案入口只携带稳定资源身份，打开时回权威系统源核对”原则延伸到 Android Calendar Provider。Tool 卡只在唯一规范事件 ID 与应用结果外壳一致时显示入口，详情页重新查询当前 Provider，并保持地点、描述、参与人和账户字段不进入 UI；没有复制参考项目的全量日历镜像、后台同步、远程协作或自动授权。

第 197 阶段把参考 Agent 常见的“答案中的稳定资源入口只负责提出目标，点击时回到权威存储核对”原则延伸到历史会话。可信会话 Tool 卡只投影唯一 `conversation-...` ID，应用在导航前重读 Room 并复用现有会话加载器；删除、重复、漂移或伪造结果保持 fail-closed。该切片没有复制参考项目的跨会话工作区、远程协作、后台自动化或多 Agent 编排。
...
第 193 阶段把参考 Agent 的“关系导航必须回到权威状态”原则落实到任务中心：UI 只从当前 Room 历史投影可确认的 `retryOfRunId` 关系生成双向入口，点击前再次核对唯一目标并只改变选中/滚动状态。历史被裁剪、关系重复或最新时间并列时保持 fail-closed；没有引入新的恢复执行、自动重试、任意 App、后台设备控制、远程 Channel 或多 Agent。

第 191 阶段将“恢复失败后重新发起”收紧为单一用户决定：只要有持久化处置证据，就必须确认创建新 Run，并在新旧证据不一致时停止。这与参考项目将恢复、重试和副作用审批分层的方式一致，但仍没有引入后台自动恢复、任意 App 操作、远程 Channel 或多 Agent。

第 192 阶段继续采用参考 Agent 的“不可变来源 Ledger + 独立新执行身份”原则：确认后只在 Room 创建带 `retryOfRunId` 的新 Run，来源终态、Step、Approval、Tool Result、回执和 Event 不被复制或改写；Repository 重建后仍以持久化事实恢复关系，新 Run 从独立空账本开始。该交叉验收没有因此开放自动恢复、旧 Executor 重放、任意 App、后台设备控制、远程 Channel 或多 Agent。

第 190 阶段把“无法恢复”从内部策略投影为用户可理解的边界：提示只给出终态数量和新 Run 重新发起说明，任务中心再展示审计证据。这延续参考实现中“恢复决策与用户操作分层”的模式，但没有因此引入新的后台执行、自动重试、任意 App 控制、远程 Channel 或多 Agent。

第 189 阶段把“终态不因重启重新打开”落实为 Room 证据：失败的日程 UPDATE 在 Repository 重建后仍不可恢复，启动收口不改写步骤、回执或事件。这与成熟 Agent 对权威 Ledger 终态和副作用重放边界的做法一致，但没有因此引入新的生产恢复通道、后台日历代理、任意 App 修改、远程 Channel 或多 Agent。

第 188 阶段用真实 Provider 漂移样本验证“审批不是事实、条件写入必须服从最新 Store”：审批落库后外部改变事件，UPDATE 被拒绝且没有提交回执，Run 失败并保留外部事实。该模式与参考项目的 fail-closed 副作用边界一致，但没有因此开放后台日历代理、任意 App 修改、重复实例或远程 Channel。

第 187 阶段把日程修改的可靠性边界落实为可复核证据：未提交路径固定 `RESTART_REQUIRED + DENY`，已提交路径在 Writer/Registry 重建后只读回查 Provider，不依赖旧进程状态，也不重复 UPDATE。该原则与参考项目中“副作用提交后以权威 Store 回读事实、恢复不猜测”的可靠性模式一致；本阶段没有因此复制后台日历代理、任意 App 修改、远程 Channel 或多 Agent 能力。

第 186 阶段用真实模型 Provider 证明“模型定位、应用绑定当前版本、审批后受控修改、Provider 回读决定事实”的完整日程链。Debug-only 探针只借用生产工具/Skill/Room Ledger，临时 Profile 精确限制三步；模型不能从文本猜 ID 或指纹，应用以 Provider 新快照确认更新后的四个字段和新版本。没有复制任意日历编辑、后台日历代理、重复实例 exception、远程 Channel 或多 Agent 能力。下一阶段优先验证失败恢复和跨进程只读确认，不横向扩大修改范围。

第 185 阶段把“模型提出补丁，应用绑定当前版本并条件提交，Provider 回读决定事实”的原则落实到日程修改。模型只能从 search/get 链取得稳定 ID 与当前指纹，字段白名单、scope、时间语义、快照漂移和写后验证由应用控制；`COMMITTED` 恢复只读且受原前台 DIRECT 边界约束。没有复制任意日历编辑、后台日历代理、重复实例 exception、远程 Channel 或多 Agent 能力。下一阶段只验证真实模型三步审批链，不横向扩大生产工具面。
...
第 181 阶段把“模型定位稳定对象、应用按身份读取权威 Store”的原则扩展到 Android Calendar Provider。`calendar.search_events` 只提供最小摘要和稳定 `Events._ID`，`calendar.get` 再从当前 Provider 单条回读；事件已删、权限变化或 Provider 异常不会回退到旧摘要。地点、描述、参与人、组织者和账户不进入类型系统，写入/删除、后台、远程 Channel 与多 Agent 也未随详情能力扩权。第 182 阶段已用真实模型 Provider 证明该两步 Skill，第 183 阶段再在独立授权边界内加入受控删除。
...
第 172 阶段沿用成熟 Agent 中“破坏性操作必须先绑定权威对象、再审批、再验证”的原则：模型只能从 search/get 结果取得 note ID，生产 Store 用 tombstone 保留防历史重放事实，Room 审批与 Tool Ledger 决定能否宣称删除成功。提交后恢复只读回读同一目标，不恢复旧协程或重放删除；没有扩展后台设备控制、任意 App、远程 Channel 或多 Agent。

第 171 阶段把“先定位稳定对象，再回到权威存储读取详情”落实为真实 Agent 闭环：模型只负责按 Skill 规划 `notes.search -> notes.get`，对象身份由 Room Tool Ledger 参数与当前 Store 交叉核对，最终回答不能替代这条证据链。Debug 夹具和临时 Profile 在完成或失败时都清理，没有把参考项目的远程通道、后台控制或任意数据访问方式带入生产。

第 170 阶段继续采用参考实现中“先定位稳定对象，再回到权威存储读取详情”的原则：`notes.get` 只接受 `notes.list / notes.search` 返回形态的 `note-UUID`，正文来自当前 Store，不从模型文本或历史摘要恢复，并明确标记为本地数据而非工具指令。不存在和 tombstone 不作可观察区分，异常旧正文有 20,000 字符上限；新增独立 SAFE Skill 以避免历史 Profile 被隐式扩权。没有复制任意 App、后台设备控制、远程 Channel 或多 Agent 能力，第 171 阶段已完成 Redmi 真实验证。

第 169 阶段继续采用参考实现中“写入成功必须有后置验证，答案动作回到权威数据源”的原则：`notes.create` 只有 Executor 回读验证成功且结果携带唯一稳定 note ID 时才提供详情入口；应用壳仍回到本地 Store 读取当前事实，不把创建结果正文升级成可编辑或可恢复权限。没有复制任意 App、后台设备控制、远程 Channel 或多 Agent 能力。

第 168 阶段沿用参考实现中“答案动作必须回到权威数据源”的原则：`notes.list / notes.search` 只有在应用生成结果能够严格解析为唯一合法 note ID 时才提供“查看笔记”；应用壳不信任摘要正文，而是进入现有本地笔记 Store 重新读取详情。多结果、伪造格式和已删除对象保持 fail-closed；没有复制任意 App、后台设备控制、远程 Channel 或多 Agent 能力。

第 167 阶段沿用参考实现中“空状态必须说明控制面边界”的原则：中断筛选没有结果时明确不会重放工具，并提供仅本地筛选级别的完整历史回退；没有引入任务执行、后台控制或高级生态能力。

第 166 阶段沿用参考实现中“恢复入口先呈现待处理事实”的原则：从启动收敛提示进入任务中心时默认筛选 `FAILED / CANCELLED`，但用户仍可回到全部历史；这只是展示投影，没有复制后台控制、任意 App 或高级生态能力。

第 165 阶段沿用参考实现中“通知只负责把用户带回权威控制面”的原则：启动恢复提示仅由应用生成的 `OperationResultAction` 提供一次性任务中心入口，点击先刷新 Agent Run 历史再导航；普通结果仍为无动作轻提示。该切片没有复制参考项目的后台设备控制、任意 App、远程 Channel 或多 Agent 能力。

第 164 阶段沿用参考实现中“恢复决策与用户通知分层”的原则：底层仍由 Room 和 `AgentRunResumePolicy` 决定恢复或收敛，UI 只在事务完成后回读失败/取消终态并显示一个脱敏汇总。通知不保存目标、错误或内部 ID，也不会驱动 Executor、旧模型协程或 Workflow 后续步骤；没有复制任意 App、后台设备控制、远程 Channel 或多 Agent 能力。

第 163 阶段沿用参考实现中“权威控制面刷新后再呈现”的原则：可信 `tasks.cancel` 终态在普通 Agent 会话状态更新并发起保存后触发一次 Workflow/ScheduledTask/Run Room 快照刷新，使任务中心与取消事实保持一致。刷新策略复用已验证取消投影，模型文本和未验证结果不能触发；没有复制参考项目的任意 App、后台设备控制、远程 Channel 或多 Agent 能力。

第 162 阶段把参考实现中“结果卡必须能回到权威任务详情”的原则应用到取消操作：`tasks.cancel` 只在已验证、应用生成的状态摘要上提供任务中心导航，点击时重新按精确名称解析当前对象。没有复制参考项目的任意 App、后台设备控制、远程 Channel 或多 Agent 能力。

第 161 阶段继续采用参考实现中“最终用户回呈必须来自已提交控制面、模型文本不能升级副作用事实”的原则。取消工具的 typed `VERIFIED` 结果现在会在普通 Agent 会话中形成一次受限终态摘要；重复调用、Workflow 来源和不完整验证都不回呈。该切片只改善取消后的可见性，没有复制参考项目的后台设备控制、任意 App、远程 Channel 或多 Agent 能力。

第 160 阶段继续采用参考实现中“危险任务操作必须显式审批、事实状态必须来自持久化控制面、取消与重试分开建模”的原则。`tasks.cancel` 只对前台直接 Agent 暴露，按精确任务名匹配唯一活动 ScheduledTask，复用停止协调器的持久化栅栏和 WorkManager 取消；它不打断前台手动 Run，也不把模型输出当成取消事实。Redmi 真实 Provider 已完成 `tasks.list -> tasks.inspect -> tasks.cancel` 并得到 `CANCELLED`，同时保持旧 Run 不变。本阶段没有复制参考项目的任意 App、后台设备动作、远程 Channel 或多 Agent 能力。

第 159 阶段继续采用参考实现中“提交成功不等于目标完成、最终回呈必须来自持久化事实”的原则。`tasks.retry` 工具只承诺关联 Run 已排队，前台宿主等 Repository 收敛后才向原会话追加完成、阻塞、失败或取消摘要；投影类型不接受内部 ID、原始错误或步骤正文。失败和取消明确不恢复旧执行栈，旧 Run 与已提交副作用继续保留。该切片只补齐用户可见结果，没有复制参考项目的后台设备控制、任意 App、远程 Channel 或任务取消扩权；Redmi 真实 Provider 复验确认 `completionVisible=true`。

第 127 至 132 阶段不再按单个原语横向扩权，而采用参考项目中更接近真实产品价值的“目标级垂直切片”。第 127 阶段已经完成自然语言目标、可确认有界计划和既有执行链复用，第 128 阶段已经完成限定 App 多动作连续执行；第 129 至 132 阶段继续完成目标级验证、记忆/知识/应用内提醒、关联恢复和 Redmi 里程碑。这样既保留 `meow-agent` 的风险元数据与后置验证、`X-OmniClaw` 的观察后执行和按需路由、`openclaw` 的任务/Channel 分层，也避免在主链跑通前复制任意 App、后台设备控制或多 Agent 的攻击面。第 132 阶段完成后，才集中评估体验、性能和高级生态。

第 128 阶段继续采用“能力声明由模型表达，权限边界由本地冻结事实决定”的原则。`target_app_package` 进入 Workflow/Run/步骤快照，Room v34 对旧记录保持 `null`；`workflow-device-action-safety-v2` 在 Prompt 之外独立约束 open、引用动作和受控导航。Runtime 不把相同 `device.snapshot` 一概视为循环，而只在上一项动作已通过验证后允许刷新，因而能形成 `观察 -> 动作 -> 新观察 -> 动作`，同时保留重复副作用门禁。Redmi 真实 production Registry 链完成 `snapshot -> swipe -> snapshot -> back`，两项动作均验证通过、零审批且最终回到小灵。这借鉴了参考项目的观察后执行与不可变任务事实，但仍只覆盖首批限定 App；最终目标是否完成留给第 129 阶段的本地证据聚合，而不是相信模型总结。
...
| 分享给小灵 | 支持 Android `ACTION_SEND` 文本、链接、图片和单份受支持文档，进入新任务草稿而非静默执行 | 分享后必须由用户确认发送；外部分享提示和附件类型可见 |
