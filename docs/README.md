...
第 86 阶段已完成 final holdout 预注册，尚未读取真实 Provider 结果：冻结 `stage85-raw-top1-qwen-v1` 及 raw top1 `0.6416276358587735`，同时冻结 Stage 85 calibration/validation 身份；新增纯 Kotlin `KnowledgeRelevanceFinalHoldoutPolicy`，只允许同 Provider/模型的第三套版本应用冻结阈值，不搜索 holdout 或重新引入 margin/z-score。`stage86-final-holdout-v1` 已固定 20 篇全新成对主题文档、三桶各 10 条英文查询和每条 2 次重复；预注册标准为正例接纳 `>=0.90`、近负例拒绝 `>=0.80`、远负例拒绝 `>=0.90`、决策稳定 `1.0`，以及 Recall@1 `>=0.90`、Recall@5 `1.0`、MRR `>=0.90`、排序稳定 `1.0`。聚焦 JVM `4/4` 与 AndroidTest APK 编译已通过；真实 Redmi 运行必须在本预注册提交后执行，结果不得用于更换语料或回调阈值。生产拒绝仍关闭。
