...
当前发布基线：`v0.1.9`；文档内容已同步到当前 `main` 工作区的 Room v16、候选记忆治理、待审批 Run 恢复、最多 4 步顺序工具闭环、声明式 Skill 管理，以及支持 1 至 8 步定义、步骤快照、可审计重试的一次性与 Daily/Weekly Workflow 实现。2026-07-19 已完成多步骤前台重试、历史快照冻结、WorkManager 后台执行、审批后继续下一步骤、Run 运行指标、模型请求 usage/TTFB/Prompt 审计、失败分布、网络中断分类和两台设备 instrumentation 验收。
