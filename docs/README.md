...
当前发布基线：`v0.1.11`；工作区文档已同步到 Room v32、Embedding 检索与显式索引重建、相关性 shadow/校准/final holdout、生产拒绝设计、第 88 阶段的来源标记/降级/引用一致性，以及第 89 阶段的生产身份状态机、真实候选探针与灰度控制面；消息 parts、Agent Profile、长期记忆、1 至 8 步 Workflow、本地知识库、答案级知识引用、设备 Agent 观察与有限动作层、网络请求独立设置页、进程退出观察账本、`/agent` Responses 附件输入，以及已迁出 ViewModel 的普通聊天上下文准备、网络发送编排、会话状态投影、保存、加载和选择/删除协调也保持有效。当前恢复边界继续 fail-closed：提交未知、验证事实不完整和旧模型协程不会原地恢复；确认后只创建关联新 Run。生产相关性身份仍为 `CANDIDATE`，生产拒绝与答案路径接入尚未启用；设备 Workflow/后台自动化、精确定时、Foreground Service、MCP、远程 Channel、多 Agent 和本地模型继续后置。
