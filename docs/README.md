...
当前发布基线：`v0.1.9`；文档内容已同步到当前 `main` 工作区的 Room v18、候选记忆治理、待审批 Run 恢复、最多 4 步顺序工具闭环、声明式 Skill 管理，以及支持 1 至 8 步定义、步骤快照、可审计重试的一次性与 Daily/Weekly Workflow 实现。2026-07-19 已完成多步骤 Workflow 真机验收、Run 请求遥测与故障注入、执行回执/幂等证据 contract、`notes.create` 验证阶段恢复，以及 `memory.remember` ToolCall 级存储幂等证明。记忆 operation ledger 以唯一幂等键绑定原始载荷哈希和 memory ID，同键重放不会新增记录，载荷漂移会被拒绝；但启动恢复白名单仍只有 `notes.create`，通用执行栈、`memory.remember` 后置验证恢复和 Workflow 后续步骤继续 fail-closed。
