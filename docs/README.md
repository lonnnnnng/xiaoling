...
当前发布基线：`v0.1.9`；文档内容已同步到当前 `main` 工作区的 Room v20、候选记忆治理、待审批 Run 恢复、最多 4 步顺序工具闭环、声明式 Skill 管理，以及支持 1 至 8 步定义、步骤快照、可审计重试的一次性与 Daily/Weekly Workflow 实现。2026-07-19 已完成多步骤 Workflow 真机验收、Run 请求遥测与故障注入、执行回执/幂等证据 contract，以及 `notes.create` 与 `memory.remember` 的受限验证阶段恢复。八类 `memory.remember` 恢复失败已通过 `run.recovery_failed` typed event 保存稳定错误码、原因和建议动作。v20 独立 `agent_tool_calls / agent_tool_results` 已成为任务中心、受限恢复和失败 Run 重试副作用判断的 Ledger-first 事实源；typed RunEvent 仅用于核对原子双写锚点，身份、字段、时间或顺序漂移均 fail-closed。账本异常时重试保守要求二次确认；账本完全为空的旧 Run 继续回退 typed RunEvent，不补造关联。Run 质量与模型遥测仍使用没有等价工具账本字段的 Step/LLM typed event 口径，通用执行栈、旧模型协程与 Workflow 后续步骤仍不恢复。
