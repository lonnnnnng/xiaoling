...
当前发布基线：`v0.1.10`；文档内容已同步到 Room v29、消息 parts、Agent Profile、长期记忆、1 至 8 步 Workflow、本地知识库、答案级知识引用、设备 Agent 观察与有限动作层、带只读诊断 UI 的独立 Android 进程退出观察账本，以及已迁出 ViewModel 的普通聊天请求上下文准备和网络发送编排。当前恢复边界保持 fail-closed：提交未知、验证事实不完整和旧模型协程不会原地恢复；确认后只创建关联新 Run。任务中心现支持“需确认”筛选，并把不能原地恢复的策略原因、稳定处置码、证据边界和下一步动作作为 typed `run.recovered` 快照直接展示。旧 typed event 的工具验证缺少 `toolCallId` 时不再按工具名或事件顺序猜配，而是明确判定恢复证据不完整。启动恢复先冻结旧 AgentRun/WorkflowRun/ScheduledTask 候选，并排除当前进程真正 `RUNNING` 的 Worker 链；用户停止会先把任务原子写为 `STOP_REQUESTED`，因此系统取消与即时 fallback 同时失败后，启动恢复仍能越过旧进程所有权并继续收敛。即使停止发生在 Worker 认领任务后、Agent Run 关联前，重入也会优先按停止栅栏取消 Workflow、未完成步骤和 Task，不会误记为执行失败。第 52 阶段又为启动恢复和确认弹窗增加 Ledger/Event canonical SHA-256 指纹，分类码相同但调用、参数、回执或验证事件漂移时拒绝旧确认并升级为 `EVIDENCE_INCOMPLETE`。第 53 阶段又把 ToolResult、执行预算与 tool.verify 三个独立提交点变成可重复故障注入边界：Result 已写入但预算快照缺失时固定拒绝原地恢复；验证事件已写入时只补控制面收尾。第 54 阶段完善模型/网络异常审计与总结兜底：规划请求异常先写失败 telemetry 后的预算快照；总结请求网络失败保留已验证工具事实并使用本地兜底回复。第 55 阶段新增 typed `llm.request.failed`：流式断流、HTTP/鉴权/限流、DNS/TLS、响应解析和未知网络异常统一落为稳定错误码，任务事件区可展示阶段、错误码和原因。第 56 阶段让普通对话在部分 delta 后断流时保留已见正文但明确标记“内容不完整”，并把失败/取消正文排除出后续请求和摘要。第 57 阶段让 Runtime 在取消收敛时先持久化单调预算，再写 Step/Run 终态，避免后台长任务丢失取消前已消耗的模型/工具时间。第 59 阶段取得约 229 秒复合只读成功样本，第 60 阶段证明冷启动链，第 61 阶段又证明熄屏状态下约 244 秒链路可完成，第 64 阶段把平台退出事实按独立、有限、隐私安全账本长期观察；当前门禁为 442 条 JVM 与仅 Redmi 执行的 152 条 instrumentation。设备工具仍只开放给前台直接 `/agent`，Workflow/后台自动化、Embedding、精确定时、Foreground Service、MCP、远程 Channel、多 Agent 和本地模型继续后置。
