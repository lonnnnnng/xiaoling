...
- 对话页
  ...
  - Workflow 设备观察现在通过 `workflow-device-observation-v1` 形成本地判断：未脱敏且未截断为“可复核”，存在脱敏或截断为“有限可复核”。下一步和关联重试都会重新回查来源 Tool Ledger，并用只含包名、计数、截断和时间的版本化判定替换模型步骤正文；证据缺失、未验证、畸形或漂移时停止后续步骤。该判定不确认节点正文、目标完成或动作授权，全部设备动作与后台设备工具仍关闭。
  ...
  - 第 112 阶段已冻结前台 Workflow 有限设备动作的 `workflow-device-action-safety-v1` 纯策略契约。策略默认不启用任何动作，只有后续显式白名单动作才会继续核验用户步骤意图、当前 Workflow/Step/AgentRun/ToolCall、同 Run 已验证 snapshot、30 秒 ref 与 window generation、当前进程逐动作审批及动作后 typed 验证；恢复、重试、取消、页面漂移和证据缺失均 fail-closed。生产 Registry 与 Accessibility 执行链尚未接线，Workflow 当前仍只能使用只读 `device.snapshot`。
