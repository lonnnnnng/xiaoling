...
- 对话页
  ...
  - 个人 Agent 主线已重新启动。用户在前台手动运行 Workflow 时，可在设备 Agent 独立开关、Accessibility 授权和 Profile/Skill 白名单均有效时使用 `device.snapshot / device.back / device.tap_ref / device.type_text`。`back` 是空参数、零审批的 SAFE 导航动作，但仍要求同 Run 新鲜观察、30 秒 TTL、当前 window generation、Executor/typed 验证和动作后重新观察；点击与输入继续逐动作走 Room/Accessibility overlay 审批。`open_app / home / swipe` 和全部后台或定时 Workflow 设备工具仍在规划清单与 Executor 两层关闭，审批恢复会从 Room 关联还原原调用来源，不能因进程重建退化成直接对话权限。
  ...
  - 第 112 阶段冻结前台 Workflow 有限设备动作的 `workflow-device-action-safety-v1` 纯策略契约；第 113 阶段接入 `tap_ref`，第 115/116 阶段完成 `type_text` 专属安全与精确回读，第 117 阶段完成 Room/overlay、答案级证据和真实 Workflow 生产闭环，第 118 阶段统一跨入口持久化隐私，第 119 阶段再接入 SAFE `back`。恢复、重试、取消、页面漂移和证据缺失继续 fail-closed，当前生产 Workflow 精确开放 `snapshot / back / tap_ref / type_text`。
  ...
  - 设备 Agent 已接入 `device.snapshot / open_app / back / home / tap_ref / type_text / swipe`：仅在用户独立开启、系统 Accessibility 已授权且 Profile/Skill 允许时可用。前台直接 `/agent` 可使用完整限定工具集；前台手动 Workflow 精确开放 `device.snapshot / device.back / device.tap_ref / device.type_text`。打开应用、点击和输入必须审批，Workflow `back` 明确为 SAFE 零审批；所有动作完成后重新观察并验证，后台或定时 Workflow 不会看到或执行任何设备工具。
...
9. 如需使用设备 Agent，在「设置 -> 设备 Agent」明确开启应用开关并完成系统无障碍授权，再为 Agent Profile 选择只读的 `device-observation` 或有限动作的 `device-control` Skill。当前只允许小灵、系统计算器、时钟和系统设置等首批白名单应用；前台手动 Workflow 可使用 `device.snapshot / device.back / device.tap_ref / device.type_text`，其他动作和全部后台设备工具仍关闭。
