...
- 对话页
  ...
  - 个人 Agent 主线已重新启动。用户在前台手动运行 Workflow 时，可在设备 Agent 独立开关、Accessibility 授权和 Profile/Skill 白名单均有效时使用 `device.snapshot / device.tap_ref / device.type_text`；后两项必须在同一 Run 重新观察、逐动作 Room/Accessibility overlay 审批并完成 Executor 与 typed 验证。`open_app / back / home / swipe` 和全部后台或定时 Workflow 设备工具仍在规划清单与 Executor 两层关闭，审批恢复会从 Room 关联还原原调用来源，不能因进程重建退化成直接对话权限。
  ...
  - Workflow 设备观察现在通过 `workflow-device-observation-v1` 形成本地判断：未脱敏且未截断为“可复核”，存在脱敏或截断为“有限可复核”。下一步和关联重试都会重新回查来源 Tool Ledger，并用只含包名、计数、截断和时间的版本化判定替换模型步骤正文；证据缺失、未验证、畸形或漂移时停止后续步骤。该观察判定本身不确认节点正文、目标完成或动作授权；`tap_ref / type_text` 仍必须另走实时 ref、逐动作审批和动作后验证，其他动作与后台设备工具继续关闭。
  ...
  - 第 112 阶段冻结前台 Workflow 有限设备动作的 `workflow-device-action-safety-v1` 纯策略契约；第 113 阶段接入 `tap_ref`，第 115/116 阶段完成 `type_text` 专属安全与精确回读，第 117 阶段再完成 Room/overlay、答案级证据和真实 Workflow 生产闭环。恢复、重试、取消、页面漂移和证据缺失继续 fail-closed，当前生产 Workflow 只开放 `snapshot / tap_ref / type_text`。
  - Workflow `type_text` 的原文只驻留当前 ToolCall；`tool.call.proposed / validated`、ToolCall ledger、Room Approval 及审批事件统一只保存 snapshot/ref、文本 SHA-256 和长度，Accessibility 浮层显示“输入 N 个字符，内容不展示”。答案级判定、下一步、关联重试和 Workflow 管理页均不保存原文、指纹或节点引用。该隐私保证限定于 Workflow 来源，直接 `/agent` 的既有会话审批与 ToolCall 审计语义本阶段不变。
  ...
  - 设备 Agent 已接入 `device.snapshot / open_app / back / home / tap_ref / type_text / swipe`：仅在用户独立开启、系统 Accessibility 已授权且 Profile/Skill 允许时可用。前台直接 `/agent` 可使用完整限定工具集；前台手动 Workflow 精确开放 `device.snapshot / device.tap_ref / device.type_text`。打开应用、点击和输入必须审批，所有动作完成后重新观察并验证；后台或定时 Workflow 不会看到或执行任何设备工具。
...
9. 如需使用设备 Agent，在「设置 -> 设备 Agent」明确开启应用开关并完成系统无障碍授权，再为 Agent Profile 选择只读的 `device-observation` 或有限动作的 `device-control` Skill。当前只允许小灵、系统计算器、时钟和系统设置等首批白名单应用；前台手动 Workflow 可使用 `device.snapshot / device.tap_ref / device.type_text`，其他动作和全部后台设备工具仍关闭。
